...
and [llama.cpp](https://github.com/ggml-org/llama.cpp). It turns interviews, lectures, meetings and
voice notes into timecoded, searchable, exportable text, and it does all of it locally on your GPU.
...
**Transcription**
- Whisper models from tiny to large-v3, GPU-accelerated with Vulkan on Linux and Windows, Metal on macOS
...
- Honest progress reporting. The `decoding`, `loading` and `transcribing` phases are reported
  separately, with a live heartbeat, so a slow file looks different from a stuck one
...
- **Summarize** writes a plain-language summary in two modes, brief or structured, generated
  locally by a Qwen3 model of your choice
- **Fix punctuation** and **Make readable** are two passes that clean up whisper's raw output line
  by line. Both return the same segments, in the same order, with the same timecodes: only the text
  ...
  endings, verb forms and word order. That matters in inflected languages, at the price of no longer
  being a literal record.
...
**Everything else**
- Batch queue: drop in several files and it transcribes them one after another
...
The app recommends a model based on the VRAM it detects, up to Large v3 on a card that can hold it,
and warns you when a model needs more memory than you have. You can always override the choice, or
point the app at your own `ggml-*.bin` file.
...
Only the Linux packages have been run by the author. CI compiles and bundles the Windows and macOS
artifacts, but nobody has launched them yet. See [Project status](#project-status).
...
- **[docs/BUILDING.md](docs/BUILDING.md)** covers prerequisites and build instructions for all
  three platforms
- **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** explains how the pipeline fits together, and why
  it is built this way
- **[CONTRIBUTING.md](CONTRIBUTING.md)** has the development workflow and house rules
...
Third-party components keep their own licenses: whisper.cpp, llama.cpp, Tauri, Symphonia, FFmpeg,
the Silero VAD model and the downloadable Whisper and Qwen models. They are all listed in
