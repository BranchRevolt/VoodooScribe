...
#
# Under Wayland a window cannot set its own icon: the compositor matches the
# window's app_id against an installed .desktop file and takes the icon from
# there, falling back to a generic placeholder when nothing matches. Packaged
# builds (.deb/.AppImage/rpm) ship their own entry; `tauri dev` does not.
#
