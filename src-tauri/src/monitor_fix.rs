...
//! Window size is otherwise left alone, apart from the startup fit-to-screen.
//!
...
mod imp {
    ...
    ///
    /// The step goes through GTK rather than `WebviewWindow::set_size`. Tauri
    /// fills `inner_size()` from the `configure-event`, which under client-side
    /// decorations includes the shadow and titlebar, while `gtk_window_resize`
    /// takes the size without them; feeding one into the other adds the frame on
    /// every call (+90x138 logical pixels per monitor change). `gtk_window.size()`
    /// and `gtk_window.resize()` share one coordinate space, so the round trip
    ...
    /// painted.
    ///
    /// Startup only: applying the same correction on every monitor change blanks
    /// the webview, so a window moved to a denser screen keeps its size and simply
    /// covers more of it.
