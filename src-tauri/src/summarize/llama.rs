...
/// `overlap_tokens` shared between neighbors. Measures the text's token density
...
/// Distinct runs of >=2 ASCII letters. In a language not written in the Latin
/// script these are foreign words the model failed to translate.
...
/// Focused corrector pass: rewrites the summary in `lang`, replacing only the
/// given stray words and leaving everything else identical.
