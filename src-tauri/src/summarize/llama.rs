...
pub fn summarize(
    ...
    let lang = dominant_language(transcript);
    ...
    if lang == Some("Russian") {
        ...
        if !strays.is_empty() {
            if let Ok(fixed) = fix_foreign_words(backend, model, &summary, &strays, cancel) {
...
/// Script-ratio guess of the transcript's dominant language, Russian vs English
/// only. None when the text has no letters.
...
fn fix_foreign_words(
    ...
    let list = words.join(", ");
    let system = format!(
        "You are a Russian proofreader. The text below is in Russian but accidentally contains a few \
         English words: {list}. Replace every such English word with its natural Russian equivalent \
         (transliterate names of people, places and brands into Cyrillic, e.g. \"presently\" → \"сейчас\"). \
         Do NOT change anything else — keep the meaning, wording, sentences and structure identical. \
         The result must contain NO Latin letters at all. Output ONLY the corrected Russian text."
    );
...
pub fn polish(
    ...
    // it.
    let system_tokens = token_len(model, prompt_template);
    ...
    for (i, chunk) in chunks.iter().enumerate() {
        ...
        let (edited, rejected) = lines::apply(chunk, &lines::parse(&answer, chunk.len()));
        if rejected > 0 {
            tracing::warn!(
