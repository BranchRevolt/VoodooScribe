...
/// Completion (success / cancel / error) is signaled via the
...
pub async fn cmd_download_model(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let event = match result {
            ...
            Err(AppError::Cancelled) => DownloadDoneEvent {
                filename: filename.clone(),
                cancelled: true,
                error: None,
            },
        ...
        tracing::info!(
            "[dl] {filename}: emitting download-done cancelled={} error={:?}",
            event.cancelled,
            event.error
        );
...
pub async fn cmd_cancel_and_delete(
    ...
    let tmp = downloader::models_dir(&app)?
        .join(&filename)
        .with_extension("tmp");
    if tmp.exists() {
