...
The sample rate and channel count come from the **decoder's** spec, not from the container header,
and that matters. HE-AAC/SBR in mp4 declares 24 kHz while actually decoding at 48 kHz;
...
Without the cap, a malformed stream spins the decoder forever.
...
null path; `apply_vad` pairs them for that reason. And whisper.cpp caches the loaded VAD model on
the `WhisperState`, so multi-chunk runs must reuse one state; creating a state per chunk reloaded
Silero dozens of times per file.

`audio/vad.rs` keeps only `split_on_silence`, the pause finder used to pick language-window
boundaries. Its silence threshold comes from the recording itself: the 20th percentile of frame
energy times three, clamped. A fixed threshold reads quiet recordings as entirely silent.
...
passes restore what whisper does badly, since its punctuation is weak on Russian even at large
sizes.
...
sentence, and are capped in *length* as well as tokens, because past a few dozen lines a 4B model
starts skipping numbers or renumbering from 1. Two consequences worth knowing:
...
- **The rules go after the transcript, not into the system turn.** With the rules first and three
  thousand tokens of speech after them, Qwen3-4B forgets the task and echoes the input back, and the
  two passes produced byte-identical output. Moving the rules behind the text made the model edit.
...
Both the Whisper context and the LLM are cached in `AppState`, because reloading Qwen3-4B on every
click cost 3–5 seconds of VRAM streaming. The llama backend is a process-wide `OnceLock` singleton,
...
- **Errors are human.** `AppError` carries a code; the frontend translates it. Errors do not
  auto-dismiss (warnings do), because a decode failure that vanished after six seconds was
  unreportable.
...
since this is device-level contention.
