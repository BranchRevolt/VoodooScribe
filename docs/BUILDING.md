...
Development happens on Linux. Windows and macOS are built by CI but have never been run by hand.
If you build there, please report what broke.
...
| A C/C++ toolchain | any | Same reason |
...
The sidecar binaries are git-ignored, since they are tens of MB each. See
...
The script is deterministic: run it without changing the logo and it reproduces the committed
icons byte for byte.

## Build troubleshooting

**A build that runs here but crashes with an illegal instruction elsewhere.** ggml enables
`GGML_NATIVE` whenever it is not cross-compiling, which compiles whisper.cpp with `-march=native`
(`/arch:AVX512` or `/arch:AVX2` under MSVC). The result is tuned to the CPU that built it.
`llama-cpp-sys-2` turns this off by itself; `whisper-rs-sys` does not, so a build meant for other
machines needs it set explicitly:

```bash
GGML_NATIVE=OFF npm run tauri build
```

The variable is forwarded into CMake by both `-sys` crates. Leaving it on is fine for a build you
